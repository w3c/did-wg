...
## Attendees
{: .no_toc}
**Present:** Manu Sporny, Samuel Smith, Brent Zundel, Daniel Burnett, Tobias Looker, Ganesh Annan, Michael Jones, Drummond Reed, Joe Andrieu, Kenneth Ebert, Yancy Ribbens, yoshiaki, Christopher Allen, David Ezell, Kaliya Young, Oliver Terbu, Justin Richer, Ivan Herman, Markus Sabadello, Jonathan Holt, Eugeniu Rusu, Phil Archer, Amy Guy

**Regrets:** 

**Guests:** Eric Welton, oskar, carsten, Michael Shea, Juan Caballero
...
> *Ivan Herman:* Meeting slides: [https://docs.google.com/presentation/d/1XI5rrEdBUSYd2tW07GfzOjBkvgKmfeKQyh95Ekl-u8o/edit#slide=id.p](https://docs.google.com/presentation/d/1XI5rrEdBUSYd2tW07GfzOjBkvgKmfeKQyh95Ekl-u8o/edit#slide=id.p)
...
**Yoshiaki Fukami:** Hi Yoshiaki Fukami, working for Data Trading Alliance, an industrial consortiom in Japan in Japan. I'm working for European Japanese reference architecture, identifiers are very important. I'm not a programmer, very difficult to choose.  
...
**Carsten** Hi Carsten from Spherity, physicist. I like DIDs because unknown actors will meet each other to establish trust using DIDs/VCs. I do a lot of C / C++, lots of simulation.  

> *Samuel Smith:* Spelled Carsten

**Oskar:** Hi Oskar from TNO, Dutch Research Institution. I'm technical coordinator for SSI arena focus on interoperability, integration, getting whole ecosystem running. I'm here to liaise with all of you. I like Java because I'm an organized guy.  
...
> *oskar:* The European project eSSIF-Lab (essif-lab.eu) funds start-ups and SMEs to build useful SSI stuff: open-source components, generic SSI functionality and services, and sector-specific SSI applications. Email: oskar.vandeventer@tno.nl
...
**juan:** Hi Juan, work with Karsten at Spherity. I like C++, Python - don't have strong opinions.  
...
> *mshea:* scribejs set mshea Michael Shea
...
**Brent Zundel:** What should we do about these issues? We could require DID Method sto follow common practice, how do we layer security, security considerations, bad practices, rubric?  
...
… Rarely do these things communicate pairwise - getout of lan onto gateway.  
...
… Interesting thing - Haystacks achieved broad industry adoption via tagged semantic model, brix project, haystacks tag ontology as overly to enable some RDF models to sit on top of haystacks. Doesn't encumber underlying encodings whie allowing any RDF standard encoding to apply to this model.  
… That's the end...  
...
… These are the things we're doing in crypcurrency space, I want to see this stuff - there is a mesh there w/ IoT, but we're doing more sophisticaed crypto.  
...
… for these JSON-LD is great. permissionless extensibility. compability with a bunch of things  
...
… Did document describes the sbuject -> JSON-LD preference  
… if DID Document contains meta-dataa for interacting with the subject -> JSON Preference  
… Similarly, do we think DIDs are part of the web?  
… are DID Documents resources on the web -> JSON-LD Preference  
… if DID Documents are mor like DNS records -> JSON preference  
...
… Let's discuss: interop, extensibiltiy, resolver behavior  
...
**Samuel Smith:** I'm not talking about hte data model, but the information model  
...
… we need to be very careful about what is expressed AND expressable in a DID Document  
...
… Because of that referenceability wrt public keys, we are almost certainly using a graph structure not a tree structure  
… 3. Graph structure?  
...
… there is a sense that if we don't have the ideal level of extensibility and all of these features, then we have the impreseion that will be a problem.  
...
… could deifnine at different levels  
… A Conceptual data model defins semantic roles and relationship without getting into syntax  
...
… Examble structural formats: EXPRESS, Protobuf, UML, IDEF1X  
...
… Idenitifiers can use names with the same meaning.  
...
**Justin Richer:** Especially with indentifiers that are not human readable.  
...
> *Kenneth Ebert:* self-issued: I would hope that service types would be registered. If not, how do implementors find out what the interoperabiliity is supposed to be?
...
### 10. DID Doc Extensibility using JSON-LD
{: #section10}
...
> *Ivan Herman:* q to ask what types of extensibility do we actually need
...
… A centralized model vs. a decentaralized model.  
...
### 11. continued Extensibility discussion
{: #section11}
...
… public domain etc  
… you can tthink of it in smaller terms  
… som vc credentials and all the way of combining data in one credential is what it's all about  
… if we have that, and this is a usecase for us, then json-ld can be used for that  
...
… infrastucture is a blockkchain  
… and you're dependanct on the distrubuted consouious  
… cryptagraphic is one that's signed  
… it's a self-cert  
… some people provide proof in a DID doc  
… that type of establishment doesn't need any extensive amount of extensibility  
… once you've establehed it  
...
… I think that's the porblem is that we need to make seperation of concerns  
...
**Manu Sporny:** I think that before we had a discussion about a layer that's pre-diddocument  
… I think the part you're concerned about is what happens before the did doc is crated  
… I think we should seperat them  
… bu I don't know if this group should sepearte them  
… I want to point out that its under the extensibiity discussion  
...
… the ledger is built aroun the mechanizm  

> *Yancy Ribbens:* mechanism

**Manu Sporny:** we merge data sources on a regular basis  
… there are many times when json-ld is usefull  
… we also have usecases where some diddocs refer to other diddocs  
...
… it's just ayone can run there own registry  

**Daniel Burnett:** from rfc 3968  
… basically there is resolution and de-referencing  
… slide 93  
… in the process of doing resolution we realized there are these things you neede to do  
...
… orginally this was just a way to give information that was asked for  
… we may have chaned our minds, but this was the original way it worked  
… that's why some of us say the format doesn't matter  
… in order to understand the de-ref you need to understand a resource  

**Joe Andrieu:** I like the seperation of authority  
...
… I need somone to inovated with a service type and a goup of white males that get to decide the future  
...
… to add you need to version teh spec\  
… they don't need to use them all but cant be changed  
… next degree is the centralized base  
… I think we could have one tha't both namespaces and key type sercvices  
...
> *Ganesh Annan:* s/teh spec\/the spec/

**Phil Archer:** my career in standards begin with a massive failure  
… because i was telling people how todo stuff  
… the one in working is successfull  
… extensibility is what makes your standered useful  
...
… so everyhthiong is forming a nice structrue  
...
… so how can we capture and strucutre that  
...
… and created a rpocess to create that core registry over time  
...
**Tobias Looker:** I want to start creating new claims beyond the did spec that will never be createed  
… or Im happy for them to be symantically unambigous  
...
… vs those that care about rotaibilty  
… and drummonds thing  
… one side is in the direction of how to rotate  
… and the other direction is how to record teh simplest stupidest client  

**Manu Sporny:** I like what im hearing  
… but it's too much meta  
… we need to get to some discussion  
… nothing wrong with the disagram  
… I'm just using this as an example  
… so that green there the registtry extensiton  
… but does the greeen thing make it impossible?  
… bu with the green stuff do you require a json-ld context  
… if that's not true we havn't solved anytihng  
...
… in will be possible without complication the working group goes to a maintance working group[  
… and I know tehre may be version issues  

> *Brent Zundel:* s/group[/group/
...
… if someone wants to make use fo tink linked data facility  
… if no so what  
...
… then that sorts everythign  

> *Juan Caballero:* (in drummond's diagram) on slide 93

**Tobias Looker:** an you update that context att the ctner  
… and because for example  
… if an open-id connect provider and they dont' publisha spec  
… its just informal by starting to emit things  
… so I don't know how it relates  

**Christopher Allen:** still havent' figured why did stuff needs to be in an endpoint  
… why am I putting them all in this dag  
… progessive thing to get to the key that they want  
...
… and its'a symantic overlay  
… and you get the best of both worlds  

### 12. Metadata
{: #section12}

**gannon:** I had the pleasure of reading the long issue  
… as well as the rfc logs  
… slide 95 is background  
… just want to say dan you've touch on the things i'm taling about  
… see slide 97  
… slide 98 did doc explained  
… the way Ilook at that is theres an object and graph model that relates to that did  
… slide 99  
...
… a did doc is not a file in a falesystem  
… so when thinkg about did document created  
… slide 102  
...
**Drummond Reed:** I want to make the point that you said antything in the did doc must describe the subject  
… thre's inforation about the did subject  
… should we seperate that  
… create date and update date  
… are we taling about when the tea pot was made and broken or not  
… how do we seprate that  

**Joe Andrieu:** couple things  
… need bear witness  
… it couldbe a bout a group which I'm a member  
… I don't like arbitrailty sucking everything up because i'm not my did  
… is the controller in charge?  
… then that' metadata  
… the reason I said on that update date  
… if a controller wants to update the date abd it took 3 days for bitcoin to get tthe tranasaction  
… . did the did conterller say it  

> *Jonathan Holt:* +1 to Joe

**Markus Sabadello:** I wanted to say something similar  
… in the resolution spec  
… in the resolution there is teh resolultion result  
… to me whats in the did doc can be different for each resolution process  
… the metadat about the persoin may be different  
… changes only when the controller updtes it  
… the second thing i wnated to say about created  
… I disagree with what gannon said  
… I thikng the did document is createx when the did is created  
… it may not be stored as a file  
… and the resolver retrives it and constructs it  

> *Kenneth Ebert:* +1 to markus' comments

**Jonathan Holt:**   
… I think time is relative  
… the time may be relavant  
… was an issue with eth classic  
… by jumping ahead in the timestamp  
… and that should be considered  

**Manu Sporny:** wanted to +1 drummond  
… there are two thihngs were talking about  
...
… markus example is outside since teh contrller didin't make that assertion  
...
… is that people want to make statemenets about two things  
...
… we might be able to define thingss about the did doc  
… and however that comes back is up to the method  
… and theres' lots with response headers  

**Phil Archer:** I like the thing about how the did doc isnt' real  
...
… nobody asked about metada about that  
...
… the resource doen't need to be real  
… so did subject and resource doesn't need to be the same thing  
… I like the idea of being clearer about sepering process  
… from information  
… was looking a did info a nd did presentation  
… clearly thrers' different levels  

**Drummond Reed:** first point I want to respinod to dan  
… we do say in the spec the did identifies the did subject  
… bucket 1  
… not sure how the did subject could  
… threre's a metapoint about identfying the did subject  
...
**Drummond Reed:** I want to adress what just says by syaing I agree with him  
… metadat describes data  
...
**Markus Sabadello:** with regard to the resouce  
...
**oskar:** some of this is confusing  
… what is the purpose fo a did document?  

**Justin Richer:** a lot of people are already deploying  
… and coming in to the group we realize there are no defintions  
… I think there are 3 different resources  
… the uri as a subject  
… once you treate as url  
… you're not getting back a subject  
… if theres a service end-point, you only get back what's seralized and returned  

**Tobias Looker:** if you put a reolver into jus a uri  
...
… and if dan says its pointing at the sky, it's whatever dan thinkgs the sky is  
… in someway it gives us the ability to describe what we thingk something is  
… the first dot is teh did controller\  
… there dot one  
...
… from a web representation, because it's metada, we have this thing markus pointed out  
… it's a resouce on the web  
...
… the thought I had was the did could be anyting  
… and if I create a document outside of did land that describes teh seating chart, that's how it will be in the did document  
… I may want you to add information about it  
… and I don't need heavy crypto  
… and I say if I created a did  
… I would have control over that and I don't need to establish authrority over that  
… it's more like a uuid  
… a lightweight did  

**Manu Sporny:** im concerned this is http range 14  
… http range 14 is a discussion onging for 20 years  
… there is no correct answer and thousands of engineers haven't found an answser  
...
**Manu Sporny:** the way you escape is someone produces some defintions  
… and if its' good enough for a few members, then we move on  
… you should read about it  

**Ivan Herman:** if you have a url against a resouce, and the resource is a book, what exectly are you referring to?  
...
**Phil Archer:** dont do the slash thing  
...
… and a defintion can evolve over time and that we pull symantic naming over dids  
… and fantastic we can get to more sophistaicated ways about symantics  

> *Yancy Ribbens:* sophisticated

**Drummond Reed:** if there are addional buckets, we can agree on the metadata  
...
… feels like there's some precuruser about what gets put in a did document  
...
… anthing in my locus of controll is in my locus of controll  
… any infomration that I put information in  
… when I see these discussions about metatdat I want to pull my hair out  

> *Manu Sporny:* 1. A DID is an identifier that identifies a resource.

> *Manu Sporny:* 2. Dereferencing the DID gives you a DID Document.

> *Manu Sporny:* 3. A DID document is the resource that is associated with a decentralized identifier (DID).

> *Manu Sporny:* 4. A DID document contains information asserted about the DID by the controller of the DID.

> *Manu Sporny:* 5. Any information that isn't asserted by the controller of the DID (i.e. metadata about the DID Document) is placed elsewhere.
...
… some times we treat is and identifer for the document  
… when we use a fragment  
… some of this is reflected here  
… the first one makes it sound like and identifer for the subject  
… 3 sound more like indifier for did document  
… are we ok with someimes we use for both  
...
… im with markus  
… the document is not the resouce  
… its information about the processing  
… im willing to say its about the subject  
...
… human is not a resouce the way the web thinks about resouce  
… so I think the way you use a tool can define that tool  
… a hammer could be a tool or a weapon  
… it echos what I said earler that we don't have clear distinctions  
...
… want to put in a plea to keep the did subject as just the resouce identified by the did  
...
… you want to do teh min for the buisness purpose  
… if its a control app that allows ocap to work  
...
… and whne you resovle tahat you end up with a 303  
...
**Phil Archer:** and drummond came up witha 4th  

**Ivan Herman:** if I have a did that I established for myself,c and I use it  
… to make assertations about my name (not only within the DID document)  
… no that did is only referring to me and never anytrhing else  
